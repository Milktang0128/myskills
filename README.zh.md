...
| v0.2 | Tauri 重构 —— 跨平台壳、Rust 后端、达到 Electron `v0.1.x` 功能 parity，并加入创造技能 preview | 活跃 preview 分支 |
...
个人项目。`v0.1.x` 是冻结的 Electron/macOS 线；`v0.2` 是 Tauri 重构线。Rust 后端已有 migration 和 scanner 不变量的单测，打包 preview app 也已经通过首屏启动 smoke。发布就绪状态跟踪在 [docs/qa/tauri-parity-smoke.md](docs/qa/tauri-parity-smoke.md)；Library、Coverage、Discover、Sync、History、Settings、AI flows 仍需要完整桌面 parity smoke 后，才能把 `v0.2.0` 当作稳定版。
