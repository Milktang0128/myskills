...
Create Skill 不要求用户先理解 input/output/schema，而是从用户的自然语言描述中提取：

```ts
{
  userJob: string,             // 用户真正想反复交给 AI 处理的工作
  triggerContext: string,      // agent 什么时候应该使用这个 skill
  inputContract: {
    acceptedInputs: string[],  // 用户可能提供的材料形态，由 Agent 推断并让用户确认
    privacyClass: 'local_only' | 'may_send_summary' | 'may_send_content'
  },
  outputContract: {
    artifactType: string,      // markdown/report/checklist/code_patch/...
...
IntentFrame 是中间产物，不直接写入 skill 目录。它可以保存在本地 DB 作为草稿与生成依据。
...
| F19 | **创造技能** | 用户用自然语言描述一个反复需求 → Agent 生成可编辑技能轮廓 → 选项式追问收窄 → 生成并审查 SKILL.md 草案 → 用户确认后写入主源并纳入 Library |
...
Agent 收到描述后，先生成一版技能轮廓，而不是立刻追问或立刻生成 `SKILL.md`。

#### 7.3.2 第二阶段：可编辑技能轮廓

技能轮廓是 Agent 对用户需求的结构化理解。它必须可编辑，用户可以直接修正理解偏差。

建议展示块：

| 模块 | 含义 |
|---|---|
| 技能目标 | 这个 skill 要稳定解决什么问题 |
| 适合输入 | Agent 推断用户通常会提供什么材料 |
| 期待输出 | 用户最终想拿到什么结果 |
| 工作方式 | skill 应该如何处理输入、分几步判断 |
| 风格偏好 | 输出语气、密度、结构、长度 |
| 边界 / 不做什么 | 不联网、不编造、不覆盖文件、信息不足先追问等 |

交互原则：

- 轮廓默认是人类可读摘要，不是技术 JSON。
- 每个块支持轻量编辑，但不进入完整 Markdown 编辑器。
- 用户修改后，Agent 重新归纳 IntentFrame。
- 只有用户确认轮廓大体正确后，才进入追问。

#### 7.3.3 第三阶段：选项式追问

追问只用于补齐会影响 skill 稳定性的关键缺口。每轮问 1 个主要问题，提供 2-4 个互斥选项，并保留“其他/补充说明”。

追问优先级：

1. 隐私与外部网络：是否允许把用户内容发送给配置的 LLM、是否允许联网查资料。
2. 文件写入：是否允许生成文件、默认写到哪里、是否允许覆盖。
3. 触发边界：什么情况下应该使用这个 skill，什么情况下不应该使用。
4. 输出格式：报告、清单、成稿、JSON、代码补丁、文件等。
5. 风格偏好：直接/严谨/锐利/温和/口语化/专业等。
...
- 目标清楚：知道它解决什么问题。
- 输入清楚：知道用户通常会给什么。
- 输出清楚：知道最终产物的形态、语气、长度或结构。
- 边界清楚：知道信息不足、联网、写文件、失败输入时如何处理。
- 至少有 2 条可检查的成功标准。
...
Create Skill 也属于 skill 目录写操作。即使是新建目录，也必须先生成创建计划，展示目标路径和 `SKILL.md` 内容摘要，用户确认后才从 staging 原子写入主源目录。创建成功后必须记录写入历史并触发重扫；若目标已存在，按冲突处理，不得静默覆盖。
