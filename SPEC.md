...
| P6 | 有反复工作流但不会沉淀成 skill | 用户知道自己有稳定需求，但不知道如何把“困境/输入/期待输出”转译成可被 agent 正确触发的 SKILL.md |
...
### 2.6 Create Skill（创造技能）

把一个反复出现的工作流创建成本地 skill，并纳入 Library、Scenario 与同步管理。

Create Skill 的核心对象不是 Markdown 文本，而是一次 **需求 → 技能规格 → 可审查产物 → 确认写入** 的生成流水线：

```
用户困境 / 需求 / 待解决问题
  → IntentFrame（结构化意图）
  → SkillSpec（技能规格）
  → Reviewed Artifact（SKILL.md 草案 + 质量检查）
  → Confirmed Write（用户确认后写入来源目录）
```

它不改变 MySkills “不做长期 Skill 编辑器”的边界：MySkills 负责引导、生成、校验、安装、纳管；创建成功后的持续精修仍交给用户常用编辑器。

#### IntentFrame（结构化意图）

Create Skill 不要求用户先理解 input/output/schema。轮廓围绕一个技能的本质——**何时用 → 输入 → 输出 → 流程 → 边界**——收敛为 5 个自由文本支柱（首屏直接编辑），加一个 `outputParts` 轻结构（防止“输出”退化成空洞描述），再加一个默认折叠、多由 Agent 推断的 `safety` 安全块：

```ts
{
  // ===== 5 支柱（首屏，自由文本 / 列表）=====
  whenToUse: string,        // agent 何时使用这个 skill（同时即 frontmatter description）
  userInput: string,        // 用户会带来什么材料、要解决什么困境/问题
  output: string,           // 这个 skill 产出什么（自由正文，构成 SKILL.md `## 输出` 段主体）
  outputParts: string[],    // 输出由哪几部分组成（轻结构，≥2 条，防空洞）
  workflow: string[],       // 工作流步骤
  boundaries: string[],     // 边界 / 不做什么（含风险操作前的确认规则）

  // ===== 折叠·高级（默认折叠，多数由 Agent 推断）=====
...
设计要点：
- **输出是自由文本，不是格式枚举。** `output` 写产物的散文描述，`outputParts` 列其可校验的组成；两者拼成 `## 输出` 段。旧的 `artifactType` 枚举降级进 `safety`，仅作安装/分类参考，不再是输出段的唯一来源。
- **safety 不在首屏出题。** Agent 据用户自然语言推断 `safety` 默认值；用户仅在折叠区按需覆盖（贯彻“首屏不做分类题”）。
- **承重保障落在产物正文，而非这些字段。** 安全审查门只扫 SKILL.md 文本里的确认/授权（gate）句；当 `safety` 声明联网/写文件/覆盖时，生成器会确定性地往 `## 安全边界` 注入对应的“先确认”规则（见 §7.3）。
- **审查门是内容卫生检查，不是执行沙箱。** 它扫注入式文案、密钥状字段、凭据路径与 exfil 形状，拦下明显危险的，但**挡不住精心伪装的恶意技能**。真正的安全边界是人：agent 经 MCP 撰写的新技能**落盘即停用**，人审后才启用；对现役技能的改写需要人在应用里确认 diff 才落盘；全程备份、可回滚。这一立场对 Create Skill 与 MCP authoring 工具一致。

IntentFrame 是中间产物，不直接写入 skill 目录。它保存在本地 DB 作为草稿与生成依据；旧形状草稿由 normalize 层惰性升级到本形状（`output` 缺失则标记为待补，强制重填一次）。
...
| F19 | **创造技能** | 用户用自然语言描述一个反复需求 → Agent 生成可编辑技能轮廓 → 选项式追问收窄 → 生成并审查 SKILL.md 草案 → 用户确认后写入来源并纳入 Library |
...
-- Create Skill 草稿（仅本地 DB，不写入 skill 目录）
CREATE TABLE skill_creation_drafts (
  id                 TEXT PRIMARY KEY,         -- uuid
  status             TEXT NOT NULL,            -- 'draft' | 'ready' | 'installed' | 'discarded'
  raw_prompt         TEXT NOT NULL,            -- 用户初始自然语言描述
  intent_frame_json  TEXT NOT NULL,            -- IntentFrame
  skill_spec_json    TEXT,                     -- 进入生成前的 SkillSpec
  draft_markdown     TEXT,                     -- 待确认的 SKILL.md
  target_platform_id TEXT REFERENCES platforms(id),
  target_scenario_id INTEGER REFERENCES scenarios(id),
  installed_skill_id TEXT REFERENCES skills(id),
  created_at         INTEGER NOT NULL,
  updated_at         INTEGER NOT NULL
);
CREATE INDEX idx_skill_creation_status ON skill_creation_drafts(status);
CREATE INDEX idx_skill_creation_updated ON skill_creation_drafts(updated_at);
```
...
### 7.3 创造技能（Create Skill）流程

Create Skill 的首屏不把用户推入“选择输入类型”的分类题。首屏只提供一个引导式大输入框，允许用户没有完整思路：

> 描述一个你反复遇到、希望交给 AI skill 处理的问题。

输入框下方提供可点击的典型需求示例。点击示例只会填入输入框，不会直接提交，用户仍可修改：
...
生产逻辑：**澄清在先、结晶在后。** 追问是逼出精度的引擎，不是线性步骤里的附加环节——Agent 先把意图问清楚，再把轮廓一次性呈现得尽可能精准，最后才生成成品。

#### 7.3.1 第一阶段：自然语言描述

用户只需要描述“困境 / 需求 / 待解决问题”。不要要求用户手工填写 input/output/schema。

Agent 收到描述后**不立刻出轮廓、也不立刻生成 `SKILL.md`**，而是先自评核心判据（见 §7.3.2）：能则直接结晶轮廓（简单需求 0 追问），不能则进入澄清循环。

#### 7.3.2 第二阶段：自适应澄清循环

**唯一停止锚点（核心判据）：「现在能否精准写出输入契约 + 输出契约（+ 何时触发）？」** —— 呼应“输入输出定清楚，技能就大致定好”。

- 每一轮 Agent 自评 `status`：
  - `ready` → 进入 §7.3.3 结晶轮廓（附完整 spec）。
  - `needs_clarification` → 返回「一行理解复述」+ **1~3 个只针对卡住输入/输出契约的**选项式问题（2-4 互斥选项 + 其他/补充）。问题是 Agent **自适应**决定的（“还差什么才能精准定义契约”），不是固定题集。
- **早抓大偏差**：每个澄清轮顶部先给「理解复述」（“我理解你想做一个把 X 变成 Y 的技能，对吗？”），用户答细节前即可纠正根本误读；并提供「重新描述」回到第一阶段。
- **防过度盘问**：
  - 简单需求第一轮即 `ready`、0 追问。
  - 只问从描述推断不出、且影响输入/输出契约的点；不问已知信息。
  - **轮数上界 3**：达上界强制结晶；用户也可随时点「够了，直接生成」强制结晶。
- **追问优先级（仅当影响契约时才问）**：输出形态（产物由哪几部分组成）> 输入边界 > 触发边界 > 隐私/联网/写文件（后者答案用于推断折叠 `safety`，不在首屏出分类题）。

示例追问：

```
当原始材料信息很散、逻辑不完整时，你希望这个 skill 怎么处理？
...
成本：简单需求 1 次 LLM 调用（第一轮即 ready）；复杂需求每轮 1 次、上界 ~3~4 次。LLM 失败/未配置时回退到确定性映射并尽量结晶。

#### 7.3.3 第三阶段：结晶轮廓（生成前唯一确认面）

澄清完成后，Agent **一次性**呈现尽可能精准的技能轮廓——“输入 → 技能 → 输出”画布，作为生成前的**唯一确认面**。用户只审这一次、可直接微调（编辑直接生效，不再被追问）。

轮廓 = 首屏 3 主体（输入 / 技能名 / 输出，输出含“包含哪些部分”要点）+ 折叠「更多细节」（何时触发 / 工作流 / 边界 / 安全 / 验收，多由 Agent 推断）：

| 模块 | 含义 | 位置 |
|---|---|---|
| 输入 | 用户带来什么材料、要解决什么困境 / 问题（needs · problem） | 首屏主体 |
| 技能名 | 目录名 / 技能名（中间的“变换”节点） | 首屏主体 |
| 输出 | 给出什么方案、产出什么结果（自由正文）+ 输出由哪几部分组成（要点列表） | 首屏主体 |
| 何时使用 | agent 何时应使用（即 frontmatter description） | 折叠·更多细节 |
| 工作流步骤 | 如何处理输入、分几步判断 | 折叠·更多细节 |
| 边界 / 不做什么 | 不联网、不编造、不覆盖文件、信息不足先追问等 | 折叠·更多细节 |
| 安全 + 验收 | 联网/写文件/覆盖/隐私/产物类型 + 验收标准 | 折叠·更多细节（多由 Agent 推断） |

交互原则：

- 轮廓是人类可读摘要，不是技术 JSON；轻量编辑，不进入完整 Markdown 编辑器。
- 不再“先给粗轮廓再追问改它”——轮廓只在澄清后出现一次。
- 确认后界面提供 **生成草案** / **继续细化**（回澄清循环）。

#### 7.3.4 第四阶段：草案预览与质量检查

草案预览分三层：

1. **摘要预览**：技能名称、用途、适合输入、期待输出。
2. **行为预览**：这个 skill 会做什么、不会做什么。
3. **技术预览**：折叠展示完整 `SKILL.md`。

质量检查必须在写入前完成：

- `name` 是安全 kebab-case，且不与现有 `(name, source_key)` 冲突。
- `description` 是触发条件，不是营销简介。
- frontmatter 可解析，不引入 MySkills 私有字段。
- `SKILL.md` 小于 scanner 上限。
- 没有默认外发用户内容。
- 没有无确认覆盖文件。
- 没有要求泄露 API key、系统提示或私密文件。
- 没有默认执行危险 shell 命令。

#### 7.3.5 第五阶段：确认安装

Create Skill 是写文件操作，必须走 plan → confirm → execute。

确认页必须展示：

- 将创建的 skill 名称与目录名。
- 目标平台 / 主源目录。
- 将写入的文件列表，MVP 只写 `SKILL.md`。
- 是否存在同名冲突。
- 是否需要备份。
- 创建后会加入哪些 Scenario。
...
Create Skill 也属于 skill 目录写操作。即使是新建目录，也必须先生成创建计划，展示目标路径和 `SKILL.md` 内容摘要，用户确认后才从 staging 原子写入来源目录。创建成功后必须记录写入历史并触发重扫；若目标已存在，按冲突处理，不得静默覆盖。
...
- Create Skill 默认 fail-closed：网络开关关闭、API key 缺失、模型输出无法解析、隐私/写入规则不明确时，不生成最终写入计划
...
- [x] macOS preview signed/notarized smoke：Developer ID 签名、staple、Gatekeeper assessment 与 DMG fixture smoke 已通过
...
- `npm run smoke:tauri:launch -- --stable-migration-smoke --frontend-smoke`（验证 stable 启动导入 Electron DB 候选流程）
